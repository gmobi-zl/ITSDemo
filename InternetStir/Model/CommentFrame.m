...
//    
    ...
    CGFloat contentLabelX = nameLabelX;
    CGFloat contentLabelY = CGRectGetMaxY(self.nameF) + padding/2;
    CGSize contentLabelSize = [MMSystemHelper sizeWithString:self.detailCommentItem.comment font:[UIFont systemFontOfSize:14] maxSize:CGSizeMake(screenW - nameLabelX - HOME_CONTENT_LEFT_PADDING, MAXFLOAT)];
    CGFloat contentLabelWidth = contentLabelSize.width;
    CGFloat contentLabelHeight = contentLabelSize.height;
    self.contentF = CGRectMake(contentLabelX, contentLabelY, contentLabelWidth, contentLabelHeight);
    ...
    if([self.detailCommentItem.replys count])
    {
        ...
        for (int i = 0; i < [self.detailCommentItem.replys count]; i++) {
            ...
            [self.replysc addObject:[NSValue valueWithCGRect:nameF]];
        }
...
-(NSMutableArray *)replyScrollF {
    if (!_replyScrollF) {
        _replyScrollF = [[NSMutableArray alloc] init];
    }
    return _replyScrollF;
}
...
//            CGRect pictureF = CGRectMake(nameLabelX, self.cellHeight, 30, 30);
            CGRect pictureF = CGRectMake(0, 0, 30, 30);

            CGFloat scrollY = self.cellHeight;
...
//            CGRect nameF = CGRectMake(nameLabelX + 40, self.cellHeight, size.width + 10, 20);
            CGRect nameF = CGRectMake( 40, 0, size.width + 10, 20);
...
//            CGRect replyF = CGRectMake(nameLabelX + 40, nameF.origin.y + 20, replyLabelWidth, replyLabelHeight);
            CGRect replyF = CGRectMake( 40, nameF.origin.y + 20, replyLabelWidth, replyLabelHeight);

            CGRect scrollF = CGRectMake(nameLabelX, scrollY, screenW - nameLabelX, 20 + replyLabelHeight);
            [self.replysF addObject:[NSValue valueWithCGRect:replyF]];
            [self.replyPictureF addObject:[NSValue valueWithCGRect:pictureF]];
            [self.replyNameF addObject:[NSValue valueWithCGRect:nameF]];
            [self.replyScrollF addObject:[NSValue valueWithCGRect:scrollF]];
            
            self.cellHeight += 18;
        }
        //评论的背景
        self.cellHeight = CGRectGetMaxY([(NSValue *)[self.replyScrollF lastObject] CGRectValue]) + padding;
        CGFloat replyBackgroundWidth = screenW - 1.5*padding - nameLabelX;
        CGFloat replyBackgroundHeight = self.cellHeight - padding*2 - CGRectGetMaxY(self.contentF);
        self.replyBackgroundF = CGRectMake(nameLabelX, CGRectGetMaxY(self.contentF) + padding, replyBackgroundWidth, replyBackgroundHeight);
    }
