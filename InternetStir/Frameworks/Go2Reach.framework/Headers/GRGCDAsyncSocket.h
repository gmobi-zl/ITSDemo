...
@class GRGCDAsyncReadPacket;
@class GRGCDAsyncWritePacket;
@class GRGCDAsyncSocketPreBuffer;
...
extern NSString *const GRGCDAsyncSocketException;
extern NSString *const GRGCDAsyncSocketErrorDomain;

extern NSString *const GRGCDAsyncSocketQueueName;
extern NSString *const GRGCDAsyncSocketThreadName;

#if SECURE_TRANSPORT_MAYBE_AVAILABLE
extern NSString *const GRGCDAsyncSocketSSLCipherSuites;
#if TARGET_OS_IPHONE
extern NSString *const GRGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const GRGCDAsyncSocketSSLProtocolVersionMax;
#else
extern NSString *const GRGCDAsyncSocketSSLDiffieHellmanParameters;
#endif
#endif

enum GRGCDAsyncSocketError
{
	GRGCDAsyncSocketNoError = 0,           // Never used
	GRGCDAsyncSocketBadConfigError,        // Invalid configuration
	GRGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	GRGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	GRGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	GRGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	GRGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	GRGCDAsyncSocketClosedError,           // The remote peer closed the connection
	GRGCDAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum GRGCDAsyncSocketError GRGCDAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface GRGCDAsyncSocket : NSObject

/**
 * GRGCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, GRGCDAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means GRGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means GRGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, GRGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, GRGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of GRGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a GRGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a GRGCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that GRGCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - GRGCDAsyncSocketSSLCipherSuites
 * - GRGCDAsyncSocketSSLProtocolVersionMin
 * - GRGCDAsyncSocketSSLProtocolVersionMax
 * 
 * If SecureTransport is available on Mac OS X:
 * 
 * - GRGCDAsyncSocketSSLCipherSuites
 * - GRGCDAsyncSocketSSLDiffieHellmanParameters;
 * 
 * 
...
/**
 * GRGCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(GRGCDAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(GRGCDAsyncSocket *)sock didAcceptNewSocket:(GRGCDAsyncSocket *)newSocket
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(GRGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol GRGCDAsyncSocketDelegate
@optional
...
**/
- (void)socket:(GRGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(GRGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(GRGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(GRGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(GRGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(GRGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(GRGCDAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(GRGCDAsyncSocket *)sock withError:(NSError *)err;
...
**/
- (void)socketDidSecure:(GRGCDAsyncSocket *)sock;
