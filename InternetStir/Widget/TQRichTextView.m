...
- (void)drawRect:(CGRect)rect
{
    ...
    {
        attString = [[self class] createAttributedStringWithText:self.text font:self.font lineSpace:self.lineSpace type:self.type];
    }
    else
    {
...
+ (NSMutableAttributedString *)createAttributedStringWithText:(NSString *)text font:(UIFont *)font lineSpace:(CGFloat)lineSpace type:(int)type
{
...
//    NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:text attributes:attributes];;
//    
    
    NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:text];
    
    CTFontRef fontRef = CTFontCreateWithName((__bridge CFStringRef)font.fontName, font.pointSize, NULL);
    [noteStr addAttribute:(NSString*)kCTFontAttributeName value:(__bridge id)fontRef range:NSMakeRange(0,noteStr.length)];
    CFRelease(fontRef);
...
//    [noteStr addAttributes:attributes range:NSMakeRange(0, [noteStr length])];
    if (type == 1) {
        [noteStr addAttributes:attributes range:NSMakeRange(0, [noteStr length])];
        ...
        NSRange replyRange = NSMakeRange([[noteStr string] rangeOfString:@"   "].location, [[noteStr string] rangeOfString:text].length - [[noteStr string]                                                                                                                                           rangeOfString:@"   "].location);
        ...
        [noteStr addAttributes:attributes range:NSMakeRange(0, [noteStr length])];
        [noteStr addAttribute:NSForegroundColorAttributeName value:[MMSystemHelper string2UIColor:HOME_COMMENT_COLOR] range:NSMakeRange(0, [noteStr length])];
    }
...
+ (CGRect)boundingRectWithSize:(CGSize)size font:(UIFont *)font string:(NSString *)string lineSpace:(CGFloat )lineSpace type:(int)type
{
    NSMutableAttributedString *attributedString = [[self class] createAttributedStringWithText:string font:font lineSpace:lineSpace type:type];
